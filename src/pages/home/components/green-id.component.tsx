...
import { greenIdTokenUrl, setTimeoutPlus } from '@/shared/utils';
...
const GreenId: FC<GreenIdInterface> = (props) => {
  ...
  const claim = async () => {
    ...
    try {
      ...
      if (success) {
        const url = greenIdTokenUrl(currentUser.greenId);
        await preload([url]);
        setNftImg(url);
  ...
  useEffect(() => {
    ...
    if (greenIdStatus == GreenIdStatusEnum.Actived) {
      setNftImg(greenIdTokenUrl(currentUser.greenId));
      if (currentUser.greenIdSpeedAmounts) {
        setBoost(currentUser.greenIdSpeedAmounts);
      }
    } else {
      setNftImg('/images/nft/notactive.png');
    }
          ...
          <div className="flex flex-col items-center justify-center w-[336px] h-[666px] lg:w-[380px] lg:h-[754px] relative">
            ...
            >
              ...
              >
                ...
                <CommonImg
                  local
                  className="w-full h-full relative z-10"
                  src={greenIdStatus == GreenIdStatusEnum.Actived ? nftImg : '/images/nft/notactive.png'}
                  alt="green id box"
                  style={{
                    backfaceVisibility: 'hidden',
                  }}
            ...
            >
              ...
              {greenIdStatus == GreenIdStatusEnum.Actived ? (
                ...
                >
                  <CommonImg local className="w-13 h-auto" src="/images/nftscan-icon.png" alt="nftscan logo" />
