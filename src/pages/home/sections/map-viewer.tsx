...
const MapViewer = (props: MapViewerProps) => {
  ...
  const initMapBg = async () => {
    // 创建图片元素
    // imgRef.current = (await preload(['/images/map/map.jpg']))[0];
    imgRef.current = (await preload([staticUrl('/forest-v3/map.jpg')]))[0];
  ...
  const initBridLight = () => {
    ...
    for (let index = 0; index < 10; index++) {
      ...
      const bird = new Two.Sprite('/images/map/bird.png', x, y, 22, 1, random(3, 10));
      bird.play();
